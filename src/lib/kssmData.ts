export const SENARAI_SUBJEK = [
  ...
  "Pendidikan Islam",
  "Pendidikan Moral",
  "Pendidikan Jasmani dan Kesihatan (PJPK)",
...
export const getSenaraiBab = (tingkatan: string, subjek: string): string[] => {
  ...
  // Fallback generic bab 1-15 jika tiada dalam senarai khusus
  return Array.from({ length: 15 }, (_, i) => `Bab ${i + 1}`);
};
...
export const generateSPOptions = (bab: string): string[] => {
  ...
  if (!match) return [];
  const babNum = match[1];
  
  const options: string[] = [];
  // Jana contoh SP yang logik (X.1.1, X.1.2 ... X.3.5)
  for (let sub = 1; sub <= 4; sub++) {
    for (let sp = 1; sp <= 5; sp++) {
      options.push(`${babNum}.${sub}.${sp}`);
    }
  }
  return options;
};
